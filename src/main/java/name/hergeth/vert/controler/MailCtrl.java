...
import name.hergeth.vert.services.MailManager;
...
public class MailCtrl {
    ...
    private VertKlasseRep klassenRep = null;
    ...
    public MailCtrl(Cfg vmConfig, VertRepository vertRepository, MailManager mm, VertKlasseRep klr){
...
//                Transport t = mailManager.initConnection(mailSendServer, mailUser, mailPassword);
                ...
                for ( Message message : messages ) {
                    ...
                    String from = addresses[0].toString();
                     if(allowedToAllSender.contains(addresses[0])){
                        ...
                        receiver = "";
                        List<VertKollege> kukList = getReceiver(subject);
                        if(kukList == null){
    ...
    private List<VertKollege> getReceiver(String subject){
        List<VertKollege> kukList = null;
        ...
        if(partialSubj.length > 1){     // there is a receiver defined
            ...
            Iterable<VertKollege> kuks = vertRepository.getKollegen();

            if(receiver.equalsIgnoreCase("ALL")){
                for (VertKollege k : kuks) {
                ...
                kit = flatten(kit);
                for (String kn : kit) {
                    Optional<VertKollege> ok = vertRepository.getKollegeByKrzl(kn);
                    if(ok.isPresent()){
                        VertKollege k = ok.get();
                        String email = k.getMailadresse();
                        if (email != null && EmailValidator.getInstance().isValid(email)){
                            kukList.add(k);
                        }

...
import name.hergeth.mailer.core.UntisDataImp;
...
public class VertRepositoryImp extends UntisDataImp implements VertRepository {
    ...
    @Override
    public Optional<Vertretung> getVertretungByVNO(Long vno){
        return vList.findByVno(vno);
    }

    @Override
    @Transactional
    public Optional<Kollege> getKollegeByKrzl(String krzl){
        return getKollegen().findByKey(krzl);
    }

    @Override
    public boolean doWithKollegen(String krzl, Function<Kollege, Boolean> func){
        Optional<Kollege> ok = getKollegen().findByKey(krzl);
        if(ok.isPresent()){
    ...
    @Override
    public List<Anrechnung> getAnrechnungenOrderByLehrerGrundBeginEnde() {

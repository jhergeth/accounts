...
//        sd = vMailerConfiguration.get("tagesStart");
    ...
    private void initFromDateTimeText(LocalDate base, LocalTime sTime, String s) {
        ...
        else if(s.equalsIgnoreCase("plus2")){
            start = base.with(TemporalAdjusters.next(DayOfWeek.MONDAY)).plusDays(7);
            end = start.with(TemporalAdjusters.next(DayOfWeek.SATURDAY));
        }
        else if(s.equalsIgnoreCase("plus3")){
            start = base.with(TemporalAdjusters.next(DayOfWeek.MONDAY)).plusDays(14);
            end = start.with(TemporalAdjusters.next(DayOfWeek.SATURDAY));
        }
        else if(s.equalsIgnoreCase("plus4")){
            start = base.with(TemporalAdjusters.next(DayOfWeek.MONDAY)).plusDays(21);
            end = start.with(TemporalAdjusters.next(DayOfWeek.SATURDAY));
        }
